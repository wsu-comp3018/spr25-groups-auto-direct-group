...
function FuelFilterModal({ isOpen, onClose, fuels, selectedFuels, onSelectionChange }) {
  const [searchTerm, setSearchTerm] = useState('');

  const handleFuelToggle = (fuel) => {
    if (selectedFuels.includes(fuel)) {
      onSelectionChange(selectedFuels.filter(f => f !== fuel));
    } else {
      onSelectionChange([...selectedFuels, fuel]);
    }
  };
  ...
  const filteredFuels = fuels.filter(fuel => 
    fuel.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Fuel Type</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {filteredFuels.length === 0 ? (
            <p className="text-gray-500 text-center py-4">No fuel types found</p>
          ) : (
            <div className="space-y-2">
              {filteredFuels.map(fuel => (
                <div key={fuel} className="flex items-center justify-between py-2 hover:bg-gray-50 rounded-lg px-2">
